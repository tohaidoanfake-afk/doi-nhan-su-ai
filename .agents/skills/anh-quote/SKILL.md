...
> ⚙️ **Bản sinh tự động cho Codex** từ `.claude/skills/anh-quote/`. Sửa ở bản gốc rồi chạy `node scripts/sinh-ban-codex.mjs`. Đừng sửa file này: lần sinh sau sẽ ghi đè.
...
Bộ máy nằm trong thư mục skill này, `.agents/skills/anh-quote/`: `khung.html` (khung gốc để copy ra kiểu mới), `dung.mjs` (render), `fonts/`, và `kieu-mau/` (sáu kiểu dựng sẵn).
...
Xem `anh-quote/kieu/` có kiểu nào khớp công thức Bước 1 chưa. Có thì dùng lại (đó là lý do có thư viện). Chưa có thì **chép một kiểu dựng sẵn** từ `.agents/skills/anh-quote/kieu-mau/` vào `anh-quote/kieu/`, hoặc **copy `khung.html` ra một file kiểu mới** rồi sửa.
...
```bash
node ".agents/skills/anh-quote/dung.mjs" --kieu anh-quote/kieu/<kiểu>.html --noi-dung anh-quote/ra/<lượt>/noi-dung.json --ra anh-quote/ra/<lượt>/anh.png
```
