...
> ⚙️ **Bản sinh tự động cho Codex** từ `.claude/skills/cham-soc/`. Sửa ở bản gốc rồi chạy `node scripts/sinh-ban-codex.mjs`. Đừng sửa file này: lần sinh sau sẽ ghi đè.
...
Khuôn bảng khách đi kèm gói: `.agents/skills/cham-soc/khuon/customers.md` — dựng vào `wiki/` qua `$nap-kho`, và đọc `reference/luat-du-lieu-nhay-cam.md` của nền trước khi điền.
