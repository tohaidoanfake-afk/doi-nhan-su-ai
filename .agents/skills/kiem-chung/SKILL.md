...
> ⚙️ **Bản sinh tự động cho Codex** từ `.claude/skills/kiem-chung/`. Sửa ở bản gốc rồi chạy `node scripts/sinh-ban-codex.mjs`. Đừng sửa file này: lần sinh sau sẽ ghi đè.
...
2. Cắm **thư mục chứa bộ não** vào làm ngữ cảnh — chính thư mục có `wiki/`, `CLAUDE.md` và `.agents/`, cả ba nằm cùng tầng.
